...
import { addDays, addMonths, monthDays, todayKey, weekDays } from './date-utils'
...
export function useTracker() {
  ...
  const goPrev = useCallback(() => {
    update((p) => {
      const step =
        p.view === 'day'
          ? addDays(p.selectedDate, -1)
          : p.view === 'week'
            ? addDays(p.selectedDate, -7)
            : addMonths(p.selectedDate, -1)
  ...
  // Week/Month: count only unpinned across the period.
  const categoryProgress = useCallback(
    (category: CategoryId): { done: number; total: number } => {
      ...
      const days = state.view === 'week' ? weekDays(state.selectedDate) : monthDays(state.selectedDate)
      let done = 0
      let total = 0
      for (const d of days) {
        const tasks = getUnpinned(category, d)
        total += tasks.length
